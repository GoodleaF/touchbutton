...
struct I2CReg
{
	...
	uint16 DiffData[CapSense_TOTAL_SENSOR_COUNT];
...
void I2CDebugInit(void)
{
     //Set RAM area to be addressed by I2C Master and '1' indicates one uint8(for button number) is writable.
    EZI2C_EzI2CSetBuffer1(sizeof(I2CRegisters), 1, (void *) &I2CRegisters);

	/* Start EZI2C (customizer's settings will be applied) */
    EZI2C_Start();

    /* Set EZI2C drive mode to internal pull up */
	EZI2C_scl_SetDriveMode(EZI2C_scl_DM_RES_UP);
	EZI2C_sda_SetDriveMode(EZI2C_sda_DM_RES_UP);
	EZI2C_scl_Write(1);
	EZI2C_sda_Write(1);
}


void WriteRawdataToInspectionReg(void)
{
    uint8 i;
	//uint16 Temp1;
    ...
    {
        #if 0
    	Temp1 = CapSense_CSD_SensorRaw[i];
		Temp1 = (uint16)(((uint32)(Temp1 * Divide[i])) >> SCALE_FACTOR);
		I2CRegisters.RawData[i] = Temp1; // Change endian for compatibility with previous host driver

		Temp1 = CapSense_CSD_SensorBaseline[i];		
		Temp1 = (uint16)(((uint32)(Temp1 * Divide[i])) >> SCALE_FACTOR);
		I2CRegisters.Baseline[i] = Temp1;	

		Temp1 = (CapSense_CSD_SensorRaw[i] > CapSense_CSD_SensorBaseline[i]) ? (CapSense_CSD_SensorRaw[i] - CapSense_CSD_SensorBaseline[i]) : 0;
		I2CRegisters.DiffData[i] = Temp1;
        #endif
        
        #if 1
        I2CRegisters.RawData[i] = Capsense_FilteredRawData[i];
        I2CRegisters.Baseline[i] = CapSense_SensorBaseline[i];	
        I2CRegisters.DiffData[i] = CapSense_SensorSignal[i];
        #endif
	}	
